...
export class Matrix {
   ...
   */
  static mul(l: NdArray, r: NdArray): NdArray {
    if (l.dimension != 2 || r.dimension != 2) throw new Error('only 2d ndarray allowed');
    if (l.isTransposed() || r.isTransposed()) throw new Error('tranposed ndarray not supported');
